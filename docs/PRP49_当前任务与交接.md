...
| **硬负样本控制** | 不排前列 | **0/2** ❌ ← 当前最大缺陷 |
| 域外泛化 | — | AUC 0.47（域特异边界，已记录）|

**硬正样本（矩阵内，直接人靶证据）**：RES-701-3×EDNRB（11 名）、RES-701-1×EDNRB（13 名）、Anantin×NPR1（23 名）
**硬负样本**：野生型 MccJ25 × ITGAV/ITGB3（实验 >10 µM 无活性）→ 模型给 0.77–0.88 ❌
