...
> ## 🏁 最终交付（2026-09-18）
>
> **FINAL 版本**：`releases/FINAL_2026-09-18_v2_full/`（31 个文件）
> 权重 `~/LassoPep/PRP49/runs_improved_grouped/checkpoints`（md5 `d0c8bcd8c6a51cab2f138ef5a4eda057`）
>
> | 指标 | 值 |
> |---|---|
> | **主判据** grouped-by-peptide CV | **0.8333 ± 0.0135** |
> | 稳健性 family-grouped CV | 0.8142 ± 0.0477 |
> | 硬门槛 | **3/3** 硬正 top-20% · **2/2** 硬负受控（**未放宽**）|
>
> **为何选 v2**：本轮无任何候选模型超过 grouped 基线（路线 A 0.7481、路线 C 0.8248 且 5/5 折为负、
> D′ 为权衡、D v2 低于 D′），三者权重相同，故按**交付完整度**择优 → v2 含靶标结构数据库、
> 三张结构优化建议表、四路线方法学边界、真实复合物读出验证。
>

> **⚠️ 适用范围（09-18 实测确定）**：本模型是**已知靶上的筛选工具，不是新靶发现工具**。
> 对三个从未见过的靶（GUK1/SSX1/EXOSC1）做零样本打分，**靶间方差仅 0.2%**、肽间 99.7%
> —— 即输出与靶无关，只是一个"肽身份常数"。与之对照，真实复合物 MccJ25×RNAP 打 9.902。
> 数据：`results/zeroshot_three_targets.csv`

> **配套文档**：`PRP49_推进总览.md`（总览）· `PRP49_版本对齐.md`（协议对齐，**引用 AUC 必附协议**）·
> `PRP49_方法学资产汇总.md`（验证阶梯 + 44 条坑）· `PRP49_ACD路线计划.md`（四路线执行结果）·
> `D3_结构优化建议.md`（工程化建议）· `target_analysis_GUK1_SSX1_EXOSC1.md`（组员提出的三靶评估）
