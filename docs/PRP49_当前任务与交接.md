...
**验收状态（冻结，09-14 10:10 更新）**
| 标准 | 目标 | 结果 |
|---|---|---|
| 主任务 CV AUC（无泄漏） | ≥ 0.75 | **0.8333** ✅ |
| 硬正样本 top-20% | ≥ 70% | **2/3（且均为 top-10%）** 🟡 |
| **硬负样本控制** | 不排前列 | **2/2 受控** ✅（42/43 → 72/73）|
| 域外泛化 | — | AUC 0.47（域特异边界，已记录）|

**硬正样本**：RES-701-3×EDNRB（11）、RES-701-1×EDNRB（10）、Anantin×NPR1（58 ❌ 无对接分）
**硬负样本**：野生型 MccJ25 × ITGAV/ITGB3 → 修复命名映射后落到 **72/73（后半区）** ✅
