...
| JobID | 名称 | 内容 | 状态 |
|---|---|---|---|
| **62632583** | `prp49_regbA` | **RMSE 方案 A 五折并行**（`--array=0-4`，每折独立作业/独立 JSON，12h 时限）| 🟢 刚提交 → 预计 **09-17 ~01:30** 完成 |
| **62629388** | `prp49_ctrft` | **路线 C 下游微调**（对比预训练编码器，config_contrastive.yaml，grouped CV）| 🟢 运行 1h |
| ✅ 完成 | `prp49_contr` 62624147 | **路线 C 家族对比预训练**（4,749 条 / 227 家族）| ✅ **15 epochs 仅 4 分 19 秒**；loss 5.24→3.53，**负对相似度 0.966→0.502**，分离度 **0.024→0.442** |
| ✅ 完成 | `prp49_regb2` 62602714_2 | **RMSE 方案 B（Kd+Ki）= 赢家** | ✅ **5/5 折完整**，RMSE **1.472** vs 基线 1.623（**改善 9.3%**），Pearson **0.484** |
| ⏱ 超时 | `prp49_regb2` 62602714_1 | RMSE 方案 A（串行）| ⏱ TIMEOUT 16:00:29，只完成 **2/5 折**（每折 7h×5=35h > 16h，坑 #36）|
| ❌ 失败 | `prp49_regb2` 62602714_3 | RMSE 方案 C | ❌ 共用 ckpt 目录崩溃（坑 #34）→ fold 0 为 1.725 vs 2.295 |
| ⏹ 取消 | `prp49_regb3` 62609719_3 | 方案 C 重提版 | ⏹ 按 60 epochs 需 27.5h，注定超时 → 主动取消 |
| ⏹ 取消 | `prp49_dockaf` 62603981 | **AF 来源对接数据集**（9 靶 × 11 肽 = 99 对）| ✅ 99/99 → 见**附录 A**（结构来源主导对接结果）|

> **新附录 B：路线 C 对比预训练（09-16）** —— 见本文档末尾。
