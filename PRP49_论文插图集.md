...
## 图 1 · Lasso 肽拓扑（**已按套马索几何重画**）

**几何关系（本次修正的核心）**：套马索不是"一个环 + 一条附着的尾巴"，而是**绳子的一端穿回
它自己形成的圈里**。相应地，图中的尾部必须**斜穿环孔**，进入段与穿出段分列环的两侧：

```
珠子 9-10（深褐） 环外·左下  —— 尾部从下方接近
珠子 11-13（深褐） 环孔内部  —— 尾部真的在环的孔里（3 个珠子）
珠子 14-16（深褐） 环外·右上  —— 穿出后向右延伸
```

- 两个交叉点已标出：**"tail enters here"**（左下）与 **"…and exits here"**（右上）
- 珠子 1-8（橙色）= 大环；红色虚线 = **异肽键**，连接珠 1（N 端 α-氨基）与珠 6
  （Asp/Glu 侧链羧基），并注明是 **N 端↔侧链**而非头尾成环
- 左中两格为对照：线性肽（可被任意切割）与普通环肽（刚性但无穿线）

**几何由断言保证**（`scratch/rebuild_fig01_v5.py`）：
尾部珠子到环珠子的最小距离 18.7 px（> 2×珠半径）；环孔内珠子数 = 3；与圆周恰好相交 2 次。

> 这一版是第 5 次迭代。前 4 版分别错在：① 环与尾画成两个独立物体 ② 尾部线条穿过标题
> ③ 漏画环肽面板的连线 ④ 尾部从环的**上缘冒出**（读起来仍像"环上挂了一条尾巴"，
> 而不是"绳端穿回自己的圈"）。**你指出的几何关系是这几次修改里最关键的一次纠正。**
